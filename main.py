    ...
    if role:
        ...
        if role and role2:
            ...
            if str(role2) == 'Computing':
                ...
                await message3.add_reaction(emoji='⌨️')
                desc = await ctx.send(f'🔥 = Ethical Hacking & Cybersecurity \n🖱️ = Computing \n⌨️ = Computer Science')
                ...
                await message3.add_reaction(emoji='🧑‍🎨')
                await message3.add_reaction(emoji='🎨')
                desc = await ctx.send(f'🖥️ = Digital Media  \n👨‍🎨 = Interior Architecture \n 🎨 = Graphic Design')
                ...
                if userDict['token'] == token:
                    ...
                    if role:
                        ...
                        if role and role2:
                            ...
                            await member.add_roles(role, role2, role3)
                            await member.remove_roles(discord.utils.get(member.guild.roles, name='Waiting for verification'))
                            await ctx.send(f"Congrats <@{userID}>! You're account is verified now! \n Welcome abroad!")
